...
interface CollapsedSidebarProps {
  ...
  // Props below are kept for compatibility but not used in simplified version
...
const CollapsedSidebar: React.FC<CollapsedSidebarProps> = ({
  ...
  const containerClasses = isMobile
    ? "fixed top-4 z-50 w-full flex px-4 gap-4 items-center"
    : "fixed top-4 left-4 z-50 flex flex-row gap-4 items-center";

  // Button styling with conditional hover effect for desktop
  const buttonBaseClasses = "p-2.5 rounded-full bg-card shadow-lg cursor-pointer transition-all duration-300 hover:shadow-xl active:scale-95";
  const sidebarButtonClasses = !isMobile && isHoveringCollapsed
    ? `${buttonBaseClasses} ring-2 ring-black dark:ring-white ring-opacity-20 dark:ring-opacity-20`
    : buttonBaseClasses;

  // Icon styling with conditional scale effect for desktop
  const iconBaseClasses = "h-5 w-5 text-adaptive transition-all duration-200";
  const sidebarIconClasses = !isMobile && isHoveringCollapsed
    ? `${iconBaseClasses} scale-110`
    : iconBaseClasses;

  return (
    <div 
      className={containerClasses}
      onMouseEnter={() => !isMobile && setIsHoveringCollapsed(true)}
      onMouseLeave={() => !isMobile && setIsHoveringCollapsed(false)}
    >
